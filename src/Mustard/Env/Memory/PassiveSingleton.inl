...
MUSTARD_ALWAYS_INLINE auto PassiveSingleton<ADerived>::Instance() -> ADerived& {
    ...
    [[unlikely]] case Base::Status::NotInstantiated:
        throw std::logic_error{PrettyException(fmt::format("{} (passive singleton in environment) has not been instantiated",
    ...
    [[unlikely]] case Base::Status::Expired:
        throw std::logic_error{PrettyException(fmt::format("The instance of {} (passive singleton in environment) has been deleted",
                                                           typeid(ADerived).name()))};
    }
    muc::unreachable();
}
