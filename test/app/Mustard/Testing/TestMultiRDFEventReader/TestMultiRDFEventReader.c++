...
TestMultiRDFEventReader::TestMultiRDFEventReader() :
    Subprogram{"TestMultiRDFEventReader", "Test Mustard::Data::RDFEventReader<T, Ms...>."} {}

auto TestMultiRDFEventReader::Main(int argc, char* argv[]) const -> int {
    ...
    const auto testMultiEventReaderReset{[&](std::array<std::string_view, 3> dataName,
        ...
        const auto checkEvtIDAlignment{[&](const auto& eventTuple, std::string_view context) -> int {
            ...
            evtIDs.reserve(3);
            const auto checkOne{[&](const auto& event) -> int {
                if (event.empty()) {
                    return EXIT_SUCCESS;
                }
                if (event[0] == nullptr) {
            ...
            if (checkOne(eventB) != EXIT_SUCCESS) {
                return EXIT_FAILURE;
            }
            if (checkOne(eventC) != EXIT_SUCCESS) {
                return EXIT_FAILURE;
            }
