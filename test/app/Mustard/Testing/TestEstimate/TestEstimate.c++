...
constexpr auto sec0Smoke{[]<int K, CovarianceOption C>() {
    ...
    // ---- free binary operators ----
    {
        ...
        // Combine
        vec = Combine(est, e3).Value();
        vec = Combine(newEst(), e3).Value();
        vec = Combine(est, newEst()).Value();
        vec = Combine(newEst(), newEst()).Value();
    }
...
auto sec0SmokeCrossType() -> void {
    ...
    // =========================================================================
    {
        ...
        scalar = Combine(ef3, ed3).Value(0);
        scalar = Combine(ed3, ef3).Value(0);
    ...
    // =========================================================================
    {
        ...
        scalar = Combine(e3f, edf3).Value(0);
        scalar = Combine(edf3, e3f).Value(0);
        ...
        scalar = Combine(e3d, edd3).Value(0);
        scalar = Combine(edd3, e3d).Value(0);
    ...
    // =========================================================================
    {
        ...
        scalar = Combine(edf3, edd3).Value(0);
        scalar = Combine(edd3, edf3).Value(0);
    ...
    // =========================================================================
    {
        ...
        scalar = Combine(e3f, edd3).Value(0);
        scalar = Combine(edd3, e3f).Value(0);
        ...
        scalar = Combine(e3d, edf3).Value(0);
        scalar = Combine(edf3, e3d).Value(0);
...
// =========================================================================
// Section 32: Combine — inverse-covariance-weighted combination (BLUE)
// =========================================================================
constexpr auto sec32Combine{[]<int K, CovarianceOption C>() {
    constexpr int dim{(K == Eigen::Dynamic) ? 3 : K};
    using Est = Estimate<K, C>;
    ...
    if constexpr (K == 1) {
        // 32a. Two identical estimates → value unchanged, variance halved
        {
            ...
            CheckClose(result.Value(), v1(0), "32a: Combine identical scalars — value unchanged");
            CheckClose(result.Variance(), cov1.diagonal()(0) / 2.0, "32a: Combine identical scalars — variance halved");
        }
        ...
        // 32b. Two different estimates
        {
            ...
            CheckClose(result.Value(), expectedVal, "32b: Combine different scalars — value");
            CheckClose(result.Variance(), expectedVar, "32b: Combine different scalars — variance");
        }
        ...
        // 32c. Self-combine via move (must run before CombineInPlace which mutates e1)
        {
            ...
            CheckClose(result.Variance(), cov1.diagonal()(0) / 2.0, "32c: Combine(move, const&) — variance halved");
        }
        ...
        // 32d. CombineInPlace
        {
            ...
            const double expectedVar{1.0 / (invVar1 + invVar2)};
            const double expectedVal{expectedVar * (invVar1 * v1(0) + invVar2 * (v1(0) + 1.0))};
            CheckClose(e1b.Value(), expectedVal, "32d: CombineInPlace scalars — value");
            CheckClose(e1b.Variance(), expectedVar, "32d: CombineInPlace scalars — variance");
        }
    } else {
        // 32a. Two identical Full/Diagonal estimates → value unchanged, variance halved
        {
            ...
            const auto result{Combine(e1, e2)};
            for (auto i{0}; i < dim; ++i) {
                CheckClose(result.Value(i), v1(i), "32a: Combine identical — value unchanged");
                CheckClose(result.Variance(i), e1.Variance(i) / 2.0, "32a: Combine identical — variance halved");
            }
            if constexpr (C == CovarianceOption::Full) {
                for (auto i{0}; i < dim; ++i) {
                    for (auto j{0}; j < dim; ++j) {
                        CheckClose(result.Covariance(i, j), e1.Covariance(i, j) / 2.0, "32a: Combine identical — covariance halved");
                    }
                }
            }
        }
        ...
        // 32b. Two different estimates — verify against BLUE formula
        {
            ...
            for (auto i{0}; i < dim; ++i) { const_cast<double&>(v2(i)) = v1(i) + 1.0; }
            ...
            if constexpr (C == CovarianceOption::Full) {
                ...
                for (auto i{0}; i < dim; ++i) {
                    CheckClose(result.Value(i), expectedVal(i), "32b: Combine different Full — value");
                }
                for (auto i{0}; i < dim; ++i) {
                    for (auto j{0}; j < dim; ++j) {
                        CheckClose(result.Covariance(i, j), expectedCov(i, j), "32b: Combine different Full — covariance");
                    }
                }
            } else {
                ...
                for (auto i{0}; i < dim; ++i) {
                    ...
                    CheckClose(result.Value(i), expectedVal, "32b: Combine different Diag — value");
                    CheckClose(result.Variance(i), expectedVar, "32b: Combine different Diag — variance");
                }
            }
        }
        ...
        // 32c. CombineInPlace
        {
            ...
            for (auto i{0}; i < dim; ++i) {
                CheckClose(eCopy.Value(i), expected.Value(i), "32c: CombineInPlace — value matches free function");
            }
        }
        ...
        // 32d. Self-combine via move
        {
            ...
            const auto result{Combine(std::move(e1), e2)};
            for (auto i{0}; i < dim; ++i) {
                CheckClose(result.Variance(i), e2.Variance(i) / 2.0, "32d: Combine(move, const&) — variance halved");
            }
        }
    }
}};
...
// =========================================================================
// Section 32b: Combine cross-type tests
// =========================================================================
auto sec32bCombineCrossType() -> void {
    ...
    // 32b-a. Combine(Full, Diagonal) → result is Full
    {
        ...
        for (auto i{0}; i < dim; ++i) {
            for (auto j{0}; j < dim; ++j) { invFull(i, j) = eF.Covariance(i, j); }
        }
        invFull = invFull.inverse().eval();
        Eigen::Matrix3d invSum{invFull};
        for (auto i{0}; i < dim; ++i) { invSum(i, i) += 1.0 / eD.Variance(i); }
        ...
        for (auto i{0}; i < dim; ++i) {
            CheckClose(result.Value(i), expectedVal(i), "32b-a: Combine(Full,Diag) value");
            CheckClose(result.Variance(i), expectedCov(i, i), "32b-a: Combine(Full,Diag) variance");
        }
    }
    ...
    // 32b-b. Combine(Diagonal, Full) → result is Full (via free function)
    {
        ...
        for (auto i{0}; i < dim; ++i) {
            CheckClose(result.Value(i), expectedVal(i), "32b-b: Combine(Diag,Full) value");
            CheckClose(result.Variance(i), expectedCov(i, i), "32b-b: Combine(Diag,Full) variance");
        }
    }
    ...
    // 32b-c. Combine(Diagonal, Diagonal) → result is Diagonal
    {
        ...
        for (auto i{0}; i < dim; ++i) {
            CheckClose(result.Variance(i), eD1.Variance(i) / 2.0, "32b-c: Combine(Diag,Diag) variance halved");
        }
    }
};
...
auto TestEstimate::Main(int argc, char* argv[]) const -> int {
    ...
    // =====================================================================
    // Section 32: Combine — inverse-covariance-weighted combination
    // =====================================================================
    PrintLn("--- Section 32: Combine ---");
    RunOverAllDims<AllStaticDims>(sec32Combine);
    PrintLn("  32 passed: Combine (K=1,2,3,5,10 Full/Diag + dynamic)");
    sec32bCombineCrossType();
    PrintLn("  32b passed: Combine cross-type");
