...
TestXoshiro512Plus::TestXoshiro512Plus() :
    Subprogram{"TestXoshiro512Plus", "Test Mustard::Random::Xoshiro512Plus and benchmark."} {}

auto TestXoshiro512Plus::Main(int /*argc*/, char* /*argv*/[]) const -> int {
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (last integer: {})", time, r);
    ...
    for (int i = 0; i < 1000; ++i) {
        std::ranges::shuffle(arr16, xoshiro512Plus);
    }
    stopwatch = {};
    for (int i = 0; i < 1'000'000; ++i) {
        std::ranges::shuffle(arr16, xoshiro512Plus);
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (first element: {})", time, arr16.front());
    ...
    for (int i = 0; i < 100; ++i) {
        std::ranges::shuffle(arr4096, xoshiro512Plus);
    }
    stopwatch = {};
    for (int i = 0; i < 10'000; ++i) {
        std::ranges::shuffle(arr4096, xoshiro512Plus);
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (first element: {})", time, arr4096.front());
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (last displacement: {})", time, v2d);
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (last displacement: {})", time, v3d);
    ...
    time = stopwatch.read();
    PrintLn("    xoshiro512+ : {} ms (last displacement: {})", time, v4d);
