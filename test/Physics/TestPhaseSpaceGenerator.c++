...
auto main(int argc, char* argv[]) -> int {
    ...
    Mustard::RAMBO<3> ramboLc2PiKP({211, -321, 2212}, {mPi, mK, mP});
    ...
    executor.Execute(nEvent, [&](auto) { FillDalitzPlot(ramboLc2PiKP, mLc, ramboLc2PiKPDalitzPlot, ramboLc2PiKPWeight); });
    executor.PrintExecutionSummary();
    WritePlot(ramboLc2PiKPDalitzPlot, ramboLc2PiKPWeight);

    Mustard::GENBOD<3> genbodLc2PiKP({211, -321, 2212}, {mPi, mK, mP});
    ...
    executor.Execute(nEvent, [&](auto) { FillDalitzPlot(genbodLc2PiKP, mLc, genbodLc2PiKPDalitzPlot, genbodLc2PiKPWeight); });
    executor.PrintExecutionSummary();
    WritePlot(genbodLc2PiKPDalitzPlot, genbodLc2PiKPWeight);

    using namespace Mustard::PhysicalConstant;
    Mustard::RAMBO<3> ramboMu2ENN({-11, -14, 12}, {electron_mass_c2, 0, 0});
    ...
    executor.Execute(nEvent, [&](auto) { FillDalitzPlot(ramboMu2ENN, muon_mass_c2, ramboMu2ENNDalitzPlot, ramboMu2ENNWeight); });
    executor.PrintExecutionSummary();
    WritePlot(ramboMu2ENNDalitzPlot, ramboMu2ENNWeight);

    Mustard::GENBOD<3> genbodMu2ENN({-11, -14, 12}, {electron_mass_c2, 0, 0});
    ...
    executor.Execute(nEvent, [&](auto) { FillDalitzPlot(genbodMu2ENN, muon_mass_c2, genbodMu2ENNDalitzPlot, genbodMu2ENNWeight); });
    executor.PrintExecutionSummary();
